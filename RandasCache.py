...
class RandasCache:
    ...
    def key_generator(self, func, *args, **kwargs):
        ...
        Returns:
            str()
        """
        params = [f"{self.leading_key}-{func.__name__}"]
        if len(args) > 0:
    ...
    def cache_float(self, func):
        ...
        def wrapper_df_decorator(*args, **kwargs):
            ...
            # check if exists in redis
            if self.cache_container.exists(key) == 1:
                # Pulls data from redis, deserialses and returns the float
                value = float(self.cache_container.get(key))
            else:
                # Runs the function that was decorated
                value = func(*args, **kwargs)

                # if function is float, insert into database
                if isinstance(value, float):
                    self.cache_container.set(key, value)
